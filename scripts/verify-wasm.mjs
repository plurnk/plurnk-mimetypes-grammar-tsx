#!/usr/bin/env node
// CI byte-identical reproducibility check for tsx.wasm.
...
const committed = await readFile(path.join(repoRoot, "tsx.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed tsx.wasm sha256: ${committedHash}`);

await using temporary = await mkdtempDisposable(path.join(tmpdir(), "grammar-tsx-verify-"));
...
console.log(`rebuilt tsx.wasm sha256: ${rebuiltHash}`);
