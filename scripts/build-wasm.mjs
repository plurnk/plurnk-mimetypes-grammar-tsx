#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-tsx.
...
await using temporary = await mkdtempDisposable(path.join(tmpdir(), "grammar-tsx-build-"));
...
const buildCwd = path.join(work, "src", "tsx");
await run(cli, ["generate"], { cwd: buildCwd });
await run(cli, ["build", "--wasm"], { cwd: buildCwd });

// Locate produced wasm and copy as tsx.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "tsx.wasm"));
const bytes = (await readFile(path.join(repoRoot, "tsx.wasm"))).length;
console.log(`tsx.wasm: ${bytes} bytes (built from ${pin})`);
